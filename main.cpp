...
void customer :: search(void)
{
	...
	while (f.read((char *) this, sizeof(customer)))
	{
		if (tcode == custcode)
		{
			...
			if (type == 'D')
				cout<<"\nType of courier : Dutiable";
			else
				cout<<"\nType of courier : Non-Dutiable";
...
void station :: addition(void)
{
	...
	gets(name);
	cout<<"Enter charges for dutiable (upto 500g)         : Rs.";
	cin>>doc;
	cout<<"Enter charges for dutiable (addition 500g)     : Rs.";
	cin>>docadd;
	cout<<"Enter charges for non-dutiable (upto 500g)     : Rs.";
	cin>>ndoc;
	cout<<"Enter charges for non-dutiable (addition 500g) : Rs.";
	cin>>ndocadd;
	do
	{
		cout<<"\nSave? (y/n) : ";
...
void station :: deletion(void)
{
	...
	while (f.read((char *) this, sizeof(station)))
	{
		if (!strcmpi(tcode,stcode))
		{
			...
			cout<<"\nCharges for dutiable (up to 500g)         : Rs."<<doc;
			cout<<"\nCharges for dutiable (addition 500g)     : Rs."<<docadd;
			cout<<"\nCharges for non-dutiable (up to 500g)     : Rs."<<ndoc;
			cout<<"\nCharges for non-dutiable (addition 500g) : Rs."<<ndocadd;
			do
			{
				cout<<"\n\nDelete? (y/n) : ";
				ch = getche();
